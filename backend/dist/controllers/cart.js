...
import { HTTPStatusBadRequest, HTTPStatusCreated, HTTPStatusOK, } from "../utils/http-response";
import { db } from "../db";
import { carts, products } from "../db/schema";
import { eq } from "drizzle-orm";
export async function addItemToCart(req, res) {
    try {
        const { data, success, error } = await validation_addCartItem.spa(req.body);
        if (!success) {
        ...
        if (isCartExist) {
            const details = await db.transaction(async (tx) => {
                ...
                const findItemInCart = cartItems.find((cartItem) => cartItem.productId === data.product_id);
                const newItemDetails = await tx.query.products.findFirst({
                    where: eq(products.id, data.product_id),
                });
                if (!newItemDetails) {
                    throw new Error("Product does not exist in store. Please try again or contact the store.");
                }
                if (data.qty + (findItemInCart?.qty ?? 0) >
            ...
            const { cartId, itemDetails } = await db.transaction(async (tx) => {
                ...
                if (!cartId) {
                    throw new Error("Failed to create a new cart. please try again later.");
                }
                return {
